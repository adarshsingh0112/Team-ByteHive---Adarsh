...
RETURN ONLY VALID JSON WITH THIS EXACT SCHEMA:
{
  ...
  (result as any).winProbability = result.winning_probability || 88;
  (result as any).critiqueText = result.scope_review?.reason || `Scope critique for ${projectIdea.substring(0, 30)}`;
  (result as any).sprintPlan = (result.sprint_plan || []).map((s: any) => ({
    phase: s.phase,
    title: s.title,
  ...
  (result as any).risks = (result.risk_analysis || []).map((r: any) => ({
    title: r.risk,
...
RETURN ONLY VALID JSON WITH THIS EXACT SCHEMA:
{
  ...
  if (aiJudgeResult) return res.json(aiJudgeResult);
